export function addWishListCard(obj, target) {
...
export function addWishListProducts(arr, target) {
  target.innerHTML = ``;
  if (arr.length) {
    arr.forEach((element) => {
      addWishListCard(element, target);
    });
  } else {
    target.innerHTML = `<h3 class="italic text-red-500 text-xl" >WishList Empty...</h3>`;
  }
}
